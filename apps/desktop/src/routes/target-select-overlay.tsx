...
type TargetUnderCursorWithElements = TargetUnderCursor & {
	element_bounds: LogicalBounds[];
};
...
function Inner() {
	...
	const [targetUnderCursor, setTargetUnderCursor] =
		createStore<TargetUnderCursorWithElements>({
			display_id: null,
			window: null,
			element_bounds: [],
		});
	...
	const unsubTargetUnderCursor = events.targetUnderCursor.listen((event) => {
		setTargetUnderCursor(
			reconcile(event.payload as TargetUnderCursorWithElements),
		);
	});
		...
		commands
			...
			.catch((e: unknown) => {
				...
				if (
					...
					toast.error(t("recording.failedToStart", { message: msg }));
				}
									...
									<div class="flex flex-col mr-2 ml-3 min-w-0">
										<span class="text-[0.95rem] font-medium text-white text-nowrap">
											{(() => {
												...
												if (startLoading()) return t("recording.preparing");
												if (rawOptions.mode === "screenshot")
							...
							<Popover.Portal>
								<Popover.Content class="z-200 w-[min(21rem,calc(100vw-1.5rem))] rounded-xl border border-amber-6 bg-gray-1 p-3.5 text-gray-12 shadow-xl outline-hidden data-expanded:animate-in data-expanded:fade-in data-expanded:zoom-in-95">
									<div class="flex gap-2.5 items-start">
										...
										<div class="flex flex-col gap-1">
											<p class="text-sm font-semibold">
												{t("recording.noMicrophoneDetected")}
											</p>
											<p class="text-xs leading-relaxed text-gray-10">
												{t("recording.noMicrophoneWarning")}
											</p>
										</div>
									</div>
									...
									<div class="flex gap-2 justify-end mt-3">
										...
										>
											{t("recording.recordWithoutMicrophone")}
										</button>
									</div>
								</Popover.Content>
							</Popover.Portal>
